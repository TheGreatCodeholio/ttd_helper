...
def zello_convert(file_path, file_name):
    subprocess.call(
        "ffmpeg -i " + file_path + "/" + file_name + ".mp3" + " -c:a libopus -b:a 64k " + file_path + "/" + file_name + ".opus",
        shell=True)
    return file_path + "/" + file_name + ".opus"
...
class ZelloSend:
    ...
    async def authenticate(self, ws):
        ...
        await ws.send_str(json.dumps({
            ...
            "username": self.config.zello_settings["username"],
            "password": self.config.zello_settings["password"],
            "channel": self.config.zello_settings["channel"]
        }))
